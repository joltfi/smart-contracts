...
library ReserveLogic {
    ...
    function _mintToTreasury(
        ...
        if (vars.amountToMint != 0) {
            uint256 treasurySplit = reserve.configuration.getTreasurySplit();
            uint256 treasuryAmount = vars.amountToMint.percentMul(
                treasurySplit
            );

            uint256 teamAmount = vars.amountToMint - treasuryAmount;

            require(teamAmount + treasuryAmount <= vars.amountToMint);
            // Mint treasury share to the treasury
            IAToken(reserve.aTokenAddress).mintToTreasury(
                treasuryAmount,
                newLiquidityIndex
            );

            // Mint team share to the team wallet
            IAToken(reserve.aTokenAddress).mintToTeam(
                teamAmount,
                newLiquidityIndex
            );
        }
