    ...
    LendingPoolStorage
{
    ...
    function liquidationCall(
        ...
        uint256 debtToCover,
        bool receiveAToken
    ) external override onlyLendingPool returns (uint256, string memory) {
        ...
        if (vars.userVariableDebt >= vars.actualDebtToLiquidate) {
            ...
            uint256 stableDebtToBurn = vars.actualDebtToLiquidate.sub(
                vars.userVariableDebt
            );
            // Ensure the user has enough stable debt to cover the remaining amount
            require(
                vars.userStableDebt >= stableDebtToBurn,
                "Insufficient stable debt"
            );
