    ...
    LendingPoolStorage
{
    ...
    // Modifier to restrict access to EOAs or whitelisted contracts
    modifier onlyEOAorWhitelist() {
        require(
            isWhitelisted[msg.sender] ||
                (_isEOA(msg.sender) && tx.origin == msg.sender),
    ...
    event WhitelistUpdated(address indexed _address, bool value);

    function whitelist(address _address, bool value) external onlyPoolOwner {
     ...
     *
     *  Modified to only allow EOA or whitelisted smart contracts to call this fn
     *
    ...
    function borrow(
        ...
        uint16 referralCode,
        address onBehalfOf
    ) external override whenNotPaused onlyEOAorWhitelist {
        DataTypes.ReserveData storage reserve = _reserves[asset];
