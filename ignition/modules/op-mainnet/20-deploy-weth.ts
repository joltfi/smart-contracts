...
module.exports = buildModule("DeployWEthModule", (m) => {
  const CONFIG = OPTIMISM_MAINNET.WETH;
  ...
  const priceFeed = m.contract("ChainlinkUniswapV3PriceFeed", [
    OPTIMISM_MAINNET.WETH.CHAINLINK,
    OPTIMISM_MAINNET.WETH.UNISWAPV3,
    60 * 30, // TWAP window
    60 * 30, // Chainlink timeout
    false,
  ]);
  ...
  const priceOracleUpdate = m.call(
    ...
    "setAssetSources",
    [[OPTIMISM_MAINNET.WETH.ASSET_ADDRESS], [priceFeed]],
    { after: [priceFeed] }
  );
    ...
    [
      {
        ...
        underlyingAsset: OPTIMISM_MAINNET.WETH.ASSET_ADDRESS,
