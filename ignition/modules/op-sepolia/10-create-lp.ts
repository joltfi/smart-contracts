...
module.exports = buildModule("CreateUniswapLP", (m) => {
  ...
  const WETH_TOKEN_AMOUNT = 100000000n; // ~$50
  ...
  const uniswapV3Factory = m.contractAt(
    "IUniswapV3Factory",
    OPTIMISM_SEPOLIA.UNISWAP_V3_FACTORY
  );
  ...
  const positionManager = m.contractAt(
    ...
    OPTIMISM_SEPOLIA.POSITION_MANAGER_ADDRESS
  );
  ...
  const approveWETH = m.call(
    wETH,
    "approve",
    [OPTIMISM_SEPOLIA.POSITION_MANAGER_ADDRESS, WETH_TOKEN_AMOUNT],
  ...
  const approveReward = m.call(
    ...
    "approve",
    [OPTIMISM_SEPOLIA.POSITION_MANAGER_ADDRESS, REWARD_TOKEN_AMOUNT],
    ...
    [
      {
        ...
        amount0Min: 0,
        amount1Min: 0,
        recipient: OPTIMISM_SEPOLIA.DEPLOYER,
        deadline,
      },
    ],
