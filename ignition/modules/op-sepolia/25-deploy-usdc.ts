...
module.exports = buildModule("DeployUSDCModule", (m) => {
  const CONFIG = OPTIMISM_SEPOLIA.USDC;
  const DECIMALS = CONFIG.DECIMALS;
  const SYMBOL = CONFIG.SYMBOL;
  const ADDRESS = CONFIG.ASSET_ADDRESS;

  // https://eth-converter.com/
  const INITIAL_DEPOSIT_AMOUNT = ethers.parseUnits("1", DECIMALS); // $2.50
  ...
  const priceFeed = m.contract("ChainlinkUniswapV3PriceFeed", [
    OPTIMISM_SEPOLIA.USDC.CHAINLINK,
    OPTIMISM_SEPOLIA.USDC.UNISWAPV3,
    60 * 30, // TWAP 30 minutes
    60 * 30, // 30 minutes heartbeat chainlink timeout
    true,
  ]);
